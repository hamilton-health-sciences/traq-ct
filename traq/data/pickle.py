...
class PickledPlate(Plate):
    ...
    def to_frame(self) -> pd.DataFrame:
        df = self._df
        if "centre" not in df.index.names:
            if "id" in df.index.names:
                id_col = "id"
            else:
                id_col = "pid"
            patient_id = df.index.get_level_values(id_col)
            centre = patient_id // 1000
            centre.name = "centre"
            df.index = [centre, patient_id]
            df.index.names = ["centre", "id"]
    ...
    def labels(self) -> np.array:
        return np.array(self._labels["record_change"].astype(int))
