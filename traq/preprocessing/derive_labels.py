...
def _pair_labels(
    df, labels_short, numeric_labels, boolean_labels, anomalous_patient_ids
):
    ...
    labels["patient_anomaly"] = False
    if anomalous_patient_ids is not None:
        labels.loc[
            labels.index.get_level_values("id").isin(anomalous_patient_ids),
            "patient_anomaly",
        ] = True
...
def _load_anomalous_patient_ids(filename):
    if filename:
        return np.array(pd.read_csv(filename)["id"])
...
def derive_diff_labels(
    ...
    anomalous_patient_ids = _load_anomalous_patient_ids(
        label_derivation_config.get("patient_anomalies", None)
    )
